...
The server validates contact and shipping fields, rechecks stock and promotions, and uses the same inventory and payment boundaries as authenticated checkout. Guests can place immediate COD orders or start a PayOS checkout reservation. A successful guest order or PayOS reservation returns a raw access token once. The client stores it only for the active success and lookup flow, masking it by default with explicit Reveal and Copy controls. The database stores only its SHA-256 hash, and public lookup requires an order ID or PayOS order code together with the token.

The server currently has no order-email provider. After an immediate order or PayOS reservation commits, authenticated customers receive the database-backed in-app order notification; guests receive the checkout-success response and can use protected guest lookup. Adding email later requires a separate server-side provider integration and must not roll back a committed order.
...
POST   /api/admin/payments/:paymentId/confirm-cod
GET    /api/admin/payments/:paymentId/webhook-events
```
...
Checkout reserves inventory before payment completion. PayOS uses a provider order code plus a server-created payment link, and its verified webhook finalization consumes a reservation once. COD creates a pending ledger payment that can be marked collected only by the guarded admin confirmation route. Return URLs only resume status polling; they never prove payment. Pending cancellation can restore inventory once; payment refunds are currently manual operator actions recorded against the payment ledger.

New catalog, checkout, order, and payment-ledger records use exact whole-number VND amounts. The server exposes `env.storeCurrency` as the VND runtime constant; there is no active FX conversion configuration. The payment ledger stores provider references, expected/settled amount and currency, reconciliation state, idempotency information, and refund state. VND catalog amounts are sent to PayOS unchanged. Historical USD orders keep their stored currency and remain readable for compatibility. Local mock mode does not call external payment APIs.

The active provider webhook endpoint is:

```text
POST /api/orders/webhooks/payos
```

PayOS live mode requires `PAYOS_CLIENT_ID`, `PAYOS_API_KEY`, and `PAYOS_CHECKSUM_KEY`. Configure the PayOS channel webhook URL to the endpoint above. Webhook envelopes are verified, stored durably, claimed idempotently, and completed with processing, ignored, mismatch, or retryable outcomes. The finalizer requires the exact VND amount, order code, and payment-link reference. The admin reconciliation run is bounded to 100 candidates per request and records append-only attempts; the admin UI can retry PayOS order payments, confirm COD collection with an optional note, and inspect linked webhook history. In local mock mode, checkout redirects to `/mock-payos-checkout` and only `POST /api/orders/mock-payos/confirm` finalizes the pending reservation after an exact amount/reference check.
