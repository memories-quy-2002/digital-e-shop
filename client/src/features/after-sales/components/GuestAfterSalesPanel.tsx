...
const GuestAfterSalesPanel = ({ orderId, guestOrderToken, currency, items }: Props) => {
    ...
    const load = async () => {
        try { setMessage(null); setExpanded(true); const page = await fetchGuestAfterSalesRequests(orderId, guestOrderToken, { page: 1, limit: 50 }); setRequests(page.requests); } catch (error) { setMessage(errorMessage(error)); }
    };
