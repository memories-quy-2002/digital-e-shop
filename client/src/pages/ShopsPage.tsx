...
} from "../components/common/AsideShops";
...
const ShopsPage = () => {
  ...
  const t = useT();
  const resetPriceRange: [number, number] = [
    facets.minPrice,
    facets.maxPrice || MAX_PRICE_RANGE,
  ];
  const hasPriceFilter =
    filters.priceRange[0] !== resetPriceRange[0] ||
    filters.priceRange[1] !== resetPriceRange[1];
  const activeFilterCount =
    filters.categories.length +
    filters.brands.length +
    (filters.term.trim() ? 1 : 0) +
    (hasPriceFilter ? 1 : 0);
  ...
  useEffect(() => {
    ...
    return () =>
      desktopQuery.removeEventListener("change", closeSheetOnDesktop);
  }, []);
  ...
  const handleCheckboxChange = useCallback(
    (type: "categories" | "brands", value: string) => {
      setFilters((prev) => ({
        ...prev,
  ...
  const removeActiveFilter = (
    type: "category" | "brand" | "term" | "price",
    value?: string,
  ) => {
    const nextFilters: Filters = {
      ...
      brands:
        type === "brand"
          ...
          : filters.brands,
      term: type === "term" ? "" : filters.term,
      priceRange: type === "price" ? resetPriceRange : filters.priceRange,
    };
    setFilters(nextFilters);
    startFilterTransition(() => updateURL(nextFilters, 1));
  };
  ...
  const activeFilterChips: {
    ...
    ...filters.categories.map((category) => ({
      ...
      label: getShopCategoryLabel(category, t),
      remove: () => removeActiveFilter("category", category),
    })),
    ...filters.brands.map((brand) => ({
      key: `brand-${brand}`,
      label: brand,
      remove: () => removeActiveFilter("brand", brand),
    })),
    ...(filters.term.trim()
      ? [
          {
            key: "term",
            label: filters.term.trim(),
            remove: () => removeActiveFilter("term"),
          },
        ]
      : []),
    ...(hasPriceFilter
  ...
  useEffect(() => {
    ...
    const priceRange = parseShopPriceRange(
        ...
        if (response.status === 200) {
          ...
          setFilters((current) => {
            if (
              ...
              current.priceRange[1] !== MAX_PRICE_RANGE
            ) {
              return current;
            }
            ...
            return {
              ...
              priceRange: [
                nextFacets.minPrice,
                nextFacets.maxPrice || MAX_PRICE_RANGE,
              ],
            };
          });
        }
    ...
    <Layout>
      ...
      <Sheet open={isMobileFiltersOpen} onOpenChange={setIsMobileFiltersOpen}>
        <div className="shops app-page">
          ...
          <div
            className="shops__toolbar"
            aria-label="Shop filters and search controls"
          >
            ...
            <div className="shops__toolbar__controls">
              <SheetTrigger asChild>
                <button
                  type="button"
                  className="shops__mobile-filter-trigger"
                  aria-label={
                    activeFilterCount > 0
                      ? `${t("shops.filters")}: ${t("shops.filterCount", activeFilterCount)}`
                      : t("shops.filters")
                  }
                >
                  {t("shops.filters")}
          ...
          {activeFilterChips.length > 0 ? (
            <div
              className="shops__active-filters"
              aria-label={t("shops.activeFilters")}
            >
              <div className="shops__active-filters-list">
                {activeFilterChips.map((chip) => (
                  <button
                    key={chip.key}
                    type="button"
                    className="shops__active-filter-chip"
                    aria-label={t("shops.removeFilter", chip.label)}
                    onClick={chip.remove}
                  >
          ...
          <SheetContent
            side="bottom"
            className="shops__filter-sheet"
            closeLabel={t("shops.closeFilters")}
          >
            <SheetHeader className="shops__filter-sheet-header">
              <SheetTitle>{t("shops.filtersTitle")}</SheetTitle>
              <SheetDescription>
                {t("shops.filterSheetDescription")}
              </SheetDescription>
            </SheetHeader>
            <div className="shops__filter-sheet-scroll">
              <AsideShops
                products={products}
                filteredCount={pagination.total}
                categories={facets.categories}
            ...
            <div className="shops__filter-sheet-footer">
              <button
                type="button"
                className="shops__filter-sheet-clear"
                onClick={handleResetFilters}
              ...
              <button
                type="button"
                className="shops__filter-sheet-apply"
