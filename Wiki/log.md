...
- 2026-09-12 - Codex - Redirected anonymous customer routes through the Firebase-backed login flow, added semantic Firebase error messages and auth form labels, locked production client config to graduation-project-5bbfb, and preserved password-reset action-code feedback in place.
- 2026-09-12 - Codex - Added anonymous cookie-backed guest-cart persistence with 30-day expiry and aggregate Admin funnel analytics, enforced owner-scoped cart mutations with stock-conflict errors, excluded canceled orders from commercial metrics, aligned status labels, and bounded Admin list queries with server pagination.
- 2026-09-13 - Codex - Made demo verification tolerate runtime login sessions above the seeded baseline and added regression coverage so local auth smoke tests do not make the demo verifier fail.
