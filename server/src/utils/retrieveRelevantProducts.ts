...
const retrieveRelevantProducts = async (productId: string | number) => {
    try {
        ...
        const documents = await collection.find({ product_id: parseInt(String(productId), 10) }).toArray();
        console.log('The ten relevant products are:');
        documents[0].relevant_products.forEach((relatedProductId: unknown, idx: number) => {
            console.log(`Product ${idx}: ${JSON.stringify(relatedProductId)}`);
        });
