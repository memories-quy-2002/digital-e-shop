...
export class ReviewsRepository {
    ...
    addReviewByUserId(uid: string, pid: number, rating: number, reviewText: string, callback: QueryCallback<UpdateResult>) {
        pool.query(
    ...
    updateReviewByUserAndProduct(uid: string, pid: number, rating: number, reviewText: string, callback: QueryCallback<UpdateResult>) {
        pool.query(
    ...
    getReviews(pid: number, callback: QueryCallback<ReviewRow[]>) {
        (async () => {
    ...
    getReviewsPaginated(pid: number, limit: number, offset: number, callback: QueryCallback<ReviewRow[]>) {
        (async () => {
            const rows = await prisma.review.findMany({
                where: { productId: pid },
                include: { user: { select: { username: true } } },
    ...
    getReviewsCount(pid: number, callback: QueryCallback<CountRow[]>) {
        (async () => {
    ...
    getRatingSummary(pid: number, callback: QueryCallback<RatingSummaryRow[]>) {
        (async () => {
