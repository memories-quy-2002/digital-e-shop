...
// AuthGuard (part of WishlistModule's DI graph) depends on NestAuthService and
// UsersRepository, both of which ultimately touch a real MySQL pool. Import
// AuthModule (which provides them, since it's @Global() in the real app but
// not auto-registered in an isolated unit test) and override both with stubs
...
describe("WishlistController", () => {
    ...
    beforeEach(async () => {
        const moduleRef = await Test.createTestingModule({
            imports: [WishlistModule, AuthModule],
        })
