...
const ensureStatusColumn = (callback) => {
    if (statusColumnReady) {
        callback();
        return;
    }

    pool.query("ALTER TABLE users ADD COLUMN status VARCHAR(20) NOT NULL DEFAULT 'Active'", (err) => {
...
const getCustomerProfile = (uid, callback) => {
    ensureStatusColumn((err) => {
        if (err) return callback(err);
        pool.query(
            `SELECT
...
const getCustomerRecentOrders = (uid, callback) => {
    pool.query(
        `SELECT
            id,
...
const updateUserAdmin = (uid, role, status, callback) => {
    ensureStatusColumn((err) => {
        if (err) return callback(err);
        pool.query(
