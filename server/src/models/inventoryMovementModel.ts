...
import type { InventoryMovementInput, InventoryMovementRow, QueryCallback, QueryParams, UpdateResult } from "../types/domain";
...
const ensureInventoryMovementTable = (callback: QueryCallback<void>) => {
    if (tableReady) {
        callback();
        return;
    }
...
const createMovement = (movement: InventoryMovementInput, callback: QueryCallback<UpdateResult> = () => {}) => {
    ensureInventoryMovementTable((err: Error | null) => {
        if (err) return callback(err);
        query(
...
const createMovements = (movements: InventoryMovementInput[], callback: QueryCallback<UpdateResult> = () => {}) => {
    ensureInventoryMovementTable((err: Error | null) => {
        if (err) return callback(err);
...
const getMovements = (limit: number, callback: QueryCallback<InventoryMovementRow[]>) => {
    ensureInventoryMovementTable((err: Error | null) => {
        if (err) return callback(err);
        query(
