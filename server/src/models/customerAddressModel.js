...
const ensureAddressTable = (callback) => {
    if (tableReady) {
        callback();
        return;
    }
...
const getAddressesByUserId = (uid, callback) => {
    ensureAddressTable((err) => {
        if (err) return callback(err);
        query(
...
const clearDefaultAddress = (uid, callback) => {
    query("UPDATE customer_addresses SET is_default = 0 WHERE user_id = ?", [uid], callback);
};

const createAddress = (uid, address, callback) => {
    ensureAddressTable((err) => {
        if (err) return callback(err);
        ...
        if (address.isDefault) {
            clearDefaultAddress(uid, (clearErr) => {
...
const updateAddress = (uid, addressId, address, callback) => {
    ensureAddressTable((err) => {
        if (err) return callback(err);
...
const deleteAddress = (uid, addressId, callback) => {
    ensureAddressTable((err) => {
        if (err) return callback(err);
