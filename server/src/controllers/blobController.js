...
async function blobHealthCheck(req, res) {
    ...
    const shouldCleanup = req.query.cleanup !== "false";
    ...
    try {
        ...
        if (shouldCleanup) {
            setTimeout(async () => {
                try {
                    ...
                    console.warn("Failed to auto-delete blob health check:", err?.message || err);
                }
            }, cleanupDelayMs);
        }
        ...
        return res.status(500).json({ ok: false, msg: err?.message || "Blob upload failed" });
    }
}
...
const uploadImage = (req, res) => {
    upload.single("file")(req, res, async (err) => {
        ...
        try {
            ...
            return res.status(500).json({ ok: false, msg: uploadErr?.message || "Upload failed" });
        }
    });
};
