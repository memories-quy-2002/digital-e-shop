...
async function addSingleProductService(data, file) {
    ...
    try {
        // Ensure brand
        let brandId;
        const brandResults = await query("SELECT id FROM brands WHERE name = ?", [brand]);
        if (brandResults.length > 0) {
            brandId = brandResults[0].id;
        } else {
            const insertBrand = await query("INSERT INTO brands (name) VALUES (?)", [brand]);
            brandId = insertBrand.insertId;
        }

        // Ensure category
        let categoryId;
        const categoryResults = await query("SELECT id FROM categories WHERE name = ?", [category]);
        if (categoryResults.length > 0) {
            categoryId = categoryResults[0].id;
        } else {
            const insertCategory = await query("INSERT INTO categories (name) VALUES (?)", [category]);
...
async function ensureNamedId(tableName, name) {
    const safeName = String(name || "").trim();
    if (!safeName) {
        throw Object.assign(new Error(`${tableName} is required`), { statusCode: 400 });
    }

    const rows = await query(`SELECT id FROM ${tableName} WHERE name = ?`, [safeName]);
    if (rows.length > 0) {
        return rows[0].id;
    }

    const result = await query(`INSERT INTO ${tableName} (name) VALUES (?)`, [safeName]);
    return result.insertId;
}

async function updateProductDetailsService(pid, updates) {
