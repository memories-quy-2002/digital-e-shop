...
const { ORDER_STATUS, USER_ROLE, PAYMENT_METHOD } = require("./constants.js");
...
async function seedOrders(customers, products) {
    ...
    for (let index = 0; index < ORDER_COUNT; index += 1) {
        ...
        const status = index % 9 === 0 ? ORDER_STATUS.CANCELED : index % 3 === 0 ? ORDER_STATUS.PENDING : ORDER_STATUS.DONE;
        const paymentMethod = index % 2 === 0 ? PAYMENT_METHOD.PAYOS : PAYMENT_METHOD.CASH;
...
async function main() {
    ...
    process.stdout.write(`Seeded ${createdOrders.length} mock orders.\n`);
    process.stdout.write(`Seeded ${createdReviews.length} mock reviews.\n`);
    process.stdout.write("Refresh Admin Dashboard, Admin Orders, Product pages, and Analytics to view the data.\n");
}

main()
    .catch((err) => {
        process.stderr.write(`Mock seed failed: ${err.message}\n`);
        process.exitCode = 1;
    })
    .finally(() => {
        pool.end();
