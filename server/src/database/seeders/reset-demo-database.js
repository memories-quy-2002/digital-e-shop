...
const importBaseline = async (connection) => {
    for (const filePath of BASELINE_FILES) {
        ...
        process.stdout.write(`Imported ${path.basename(filePath)}\n`);
    }
};
...
const main = async () => {
    ...
    try {
        ...
        try {
            ...
            process.stdout.write(`Dropped ${droppedTables.length} existing base tables from the selected database.\n`);
            await importBaseline(connection);
            const clearedRows = await clearAllTableData(connection);
            process.stdout.write(`Cleared ${clearedRows} baseline rows before running the demo seed.\n`);
        } finally {
            connection.release();
        }
    } finally {
...
if (require.main === module) {
    main().catch((error) => {
        process.stderr.write(
            `Digital-E demo database reset failed: ${error instanceof Error ? error.code || error.message || "unknown error" : String(error)}\n`,
        );
        process.exitCode = 1;
    });
}
