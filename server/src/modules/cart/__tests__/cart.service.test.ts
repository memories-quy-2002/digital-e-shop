...
import type { CartItemRow, CartValidationIssue, CheckoutMismatch } from "../cart.types";
import type { CartCheckoutItem } from "../cart.dto";

// cart.service require()s its repositories at load. Mock them so importing the
// service never touches the database; the functions under test are pure.
vi.mock("../cart.repository", () => ({}));
vi.mock("#src/modules/products/products.repository", () => ({}));

import cartService from "../cart.service";

const { buildCartValidationIssue, buildCartValidationResult, compareSubmittedCart } = cartService as {
    buildCartValidationIssue: (item: CartItemRow) => CartValidationIssue | null;
    buildCartValidationResult: (items: CartItemRow[]) => {
        valid: boolean;
        cartItems: CartItemRow[];
        issues: CartValidationIssue[];
    };
    compareSubmittedCart: (
        authoritative: CartItemRow[],
        submitted: CartCheckoutItem[],
